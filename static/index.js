...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 15) return '跟个二百五一样';
    if (score <= 30) return '怪我喽';
    if (score <= 45) return '没反应啊';
    if (score <= 66) return '你算哪块小饼干';
    if (score <= 78) return '我要身败名裂了';
    if (score <= 88) return '来来来来来来';
    if (score <= 91) return '怎么';
    return '为什么我知道呢？';
}
...
function cookie(name, value, time) {
    if (name) {
        ...
        return value = document.cookie.match("(?:^|;)\\s*" + name.replace(/([-.*+?^${}()|[\]\/\\])/g, "\\$1") + "=([^;]*)"),
            value = value && "string" == typeof value[2] ? unescape(value[1]) : !1, (/^(\{|\[).+\}|\]$/.test(value) ||
...
function initSetting() {
    ...
    if(cookie("keyboard")){
        ...
        map={}
        map[cookie("keyboard").charAt(0).toLowerCase()]=f;
        map[cookie("keyboard").charAt(1).toLowerCase()]=g;
        map[cookie("keyboard").charAt(2).toLowerCase()]=h;
        map[cookie("keyboard").charAt(3).toLowerCase()]=j;
    }
}
function show_btn() {
